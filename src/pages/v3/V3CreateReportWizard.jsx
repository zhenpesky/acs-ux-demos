...
function buildLiveResultUrl(basePath, chipTexts) {
...
function ResourcesStep({ formValues, updateFormValue, resultsPath }) {
    ...
    <>
      ...
      <Form style={{ padding: '24px' }}>
        ...
        {formValues.scopeMethod === 'custom-filters' && (() => {
          const chips = formValues.scopeChips || [];
          const addChip = (chip) => {
            updateFormValue('scopeChips', [...chips, chip]);
          };
          const clearAll = () => {
            updateFormValue('scopeChips', []);
          };
          const liveResultUrl = buildLiveResultUrl(resultsPath, chips);

          return (
            <FormGroup fieldId="define-scope" style={{ marginTop: 16 }}>
              <ScopeDefineToolbar onAddChip={addChip} liveResultUrl={liveResultUrl} />
              {chips.length > 0 && (() => {
                const allItems = chips.map((chip, idx) => ({ key: `c-${idx}`, text: chip, idx }));
                const grouped = groupChipsByCategory(allItems);
                const removeItem = (item) => {
                  updateFormValue('scopeChips', chips.filter((_, i) => i !== item.idx));
                };
                const removeCategory = (category) => {
                  const removedIdxs = new Set(grouped[category].map((i) => i.idx));
                  updateFormValue('scopeChips', chips.filter((_, i) => !removedIdxs.has(i)));
                };
                return (
                  <Flex gap={{ default: 'gapSm' }} flexWrap={{ default: 'wrap' }} alignItems={{ default: 'alignItemsCenter' }} style={{ marginTop: 12 }}>
                    ...
                    <FlexItem>
                      <Content component={ContentVariants.small}>{chips.length} filters applied</Content>
                    </FlexItem>
                    <FlexItem>
...
export default function V3CreateReportWizard({ routePrefix = '/v3' }) {
  useLiveSync();
  ...
  const [formValues, setFormValues] = useState(() => {
    ...
    const filterLabel = incomingSavedFilterName && !incomingFiltersModified
      ? `${incomingSavedFilterName} - ${dateStr} ${timeStr}`
      : `Vulnerability Report - ${dateStr} ${timeStr}`;
    defaults.name = filterLabel;
  ...
  const handleSave = () => {
    const collectionName = formValues.scopeMethod === 'collection'
      ? COLLECTIONS.find((c) => c.id === formValues.selectedCollection)?.name || ''
      : '';
    const reportData = {
      name: formValues.name,
      description: formValues.description,
      collection: collectionName || 'Custom filters',
      ...
      scopeFilters: formValues.scopeChips || [],
    };
  ...
  const isStep2Valid = (
    formValues.scopeMethod === 'collection' ? formValues.selectedCollection !== null : true
  );
