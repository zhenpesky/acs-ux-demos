...
const VULN_STATE_LABELS = { OBSERVED: "Observed", DEFERRED: "Deferred", FALSE_POSITIVE: "False positives" };
    ...
    return (
      <>
        <ToolbarItem className="pf-v6-u-flex-grow-1">
            ...
            toggle={(ref) => (
              <MenuToggle ref={ref} variant="typeahead" onClick={() => setSuggestionsOpen(!suggestionsOpen)} isExpanded={suggestionsOpen} className="pf-v6-u-w-100">
                <TextInputGroup isPlain>
                  ...
                  <TextInputGroupUtilities>
                    {inputValue && (
                      <Button variant="plain" onClick={() => { setInputValue(""); setSuggestionsOpen(false); }} aria-label="Clear"><TimesIcon /></Button>
                    )}
          ...
          >
            <SelectList>
              {suggestions.slice(0, 15).map((s) => <SelectOption key={s} value={s}>{s}</SelectOption>)}
            </SelectList>
          </Select>
        </ToolbarItem>
      </>
...
function ManageSavedFiltersModal({
  isOpen,
  onClose,
  items,
  onReorder,
...
export default function UserWorkloadVulnerabilities({ routePrefix = '/v1' }) {
  useLiveSync();
  ...
  const [baselineFilters, setBaselineFilters] = useState(null);
  ...
  const { data: deployments, loading } = useWorkloadCVEs(activeFilters);
  const { data: clusters } = useClusters();
  const { data: vulnSummary } = useVulnerabilitySummary();
  ...
  const [manageFilterItems, setManageFilterItems] = useState([]);
  useEffect(() => {
    if (savedFilters.length > 0 && manageFilterItems.length === 0) {
      setManageFilterItems(savedFilters.map((sf) => ({ id: sf.id, name: sf.name, hidden: false })));
    }
  }, [savedFilters]);

  const filteredDeployments = useMemo(() => {
    const all = deployments?.length ? deployments : DEPLOYMENTS;
    if (vulnState === "OBSERVED") return all;
    if (vulnState === "DEFERRED") return all.filter((_, i) => i % 3 === 0);
    return all.filter((_, i) => i % 5 === 0);
  }, [deployments, vulnState]);

  const deploymentCveTotal = (d) => {
  ...
  const sortedDeployments = useMemo(() => {
    ...
    list.sort((a, b) => {
      switch (sortIndex) {
        ...
        case 1:
          return dir * (deploymentCveTotal(a) - deploymentCveTotal(b));
        case 2:
          return dir * String(a.clusterName || "").localeCompare(String(b.clusterName || ""), undefined, { sensitivity: "base" });
        case 3:
          return dir * String(a.namespace || "").localeCompare(String(b.namespace || ""), undefined, { sensitivity: "base" });
        case 4:
          return dir * ((a.imageCount || 0) - (b.imageCount || 0));
        case 5: {
  ...
  const totalImages = useMemo(() =>
    filteredDeployments.reduce((acc, d) => acc + (d.imageCount || 0), 0),
    [filteredDeployments]);
  ...
  function removeFilter(key, val) { setActiveFilters((prev) => ({ ...prev, [key]: prev[key].filter((v) => v !== val) })); setPage(1); }
  function clearAllFilters() { setActiveFilters({ ...emptyFilters }); setSelectedSavedFilter(null); setBaselineFilters(null); setPage(1); }
  function toggleFilter(key, value) { setActiveFilters((prev) => ({ ...prev, [key]: prev[key].includes(value) ? prev[key].filter((x) => x !== value) : [...prev[key], value] })); setPage(1); }

  const filtersModified = selectedSavedFilter ? !filtersEqual(activeFilters, baselineFilters) : false;
  ...
  function applySavedFilter(filter) {
    ...
    const newFilters = {
      ...emptyFilters,
      clusterName: filter.filters?.clusterName || [],
      namespaceName: filter.filters?.namespaceName || [],
      deploymentName: filter.filters?.deploymentName || [],
      cveSeverity: filter.filters?.cveSeverity || filter.filters?.severity || [],
      cveStatus: filter.filters?.cveStatus || [],
    };
    setActiveFilters(newFilters);
    setBaselineFilters({ ...newFilters });
    setPage(1);
  }

  function handleDefaultFiltersApply(sevs, fixable) {
    setActiveFilters((prev) => ({ ...prev, cveSeverity: sevs, cveStatus: fixable ? ["Fixable"] : [] }));
    setPage(1);
  }
  ...
  async function handleSaveFilter() {
    if (newFilterName.trim()) {
      await createFilter({ name: newFilterName, description: newFilterDescription, filters: activeFilters, createdBy: "user@example.com" });
  ...
  return (
    <>
          ...
          <FlexItem>
            ...
            >
              <DropdownList>
                ...
                <DropdownItem key="no-cves" onClick={() => navigate(routes.vulnerabilitiesUserWorkloadsPath)} description="Images and workloads without observed CVEs (results may include false negatives due to scanner limitations, such as unsupported operating systems)">Images without CVEs</DropdownItem>
      ...
      {/* Page heading + vulnerability state tabs */}
      ...
      <PageSection padding={{ default: "noPadding" }}>
        <Tabs
          activeKey={vulnState}
          onSelect={(_e, k) => handleVulnStateChange(k)}
          usePageInsets
          isBox
        >
          {VULN_STATES.map((s) => (
      ...
      <PageSection>
        {/* Compound search filter toolbar */}
        <Toolbar>
          <ToolbarContent>
            ...
            <ToolbarItem>
              <Select
                ...
                toggle={(ref) => (
                  <MenuToggle ref={ref} onClick={() => setSavedFiltersOpen(!savedFiltersOpen)} isExpanded={savedFiltersOpen} icon={<FilterIcon />}>
                    {selectedSavedFilter || "Saved filters"}
                  </MenuToggle>
                )}
              >
                <SelectList>
                ...
                <SelectList>
                  {savedFilters.map((sf) => (
                    <SelectOption key={sf.id} onClick={() => applySavedFilter(sf)}>
                      <div>
                        <div style={{ fontWeight: 500 }}>{sf.name}</div>
                        <div style={{ fontSize: 12, color: "var(--pf-t--global--text--color--subtle)" }}>{sf.description}</div>
                      </div>
                    </SelectOption>
                  ))}
                </SelectList>
            ...
            <ToolbarGroup variant="filter-group">
              <ToolbarItem>
                <Select isOpen={severityOpen} onOpenChange={setSeverityOpen}
                  toggle={(ref) => (
                    <MenuToggle ref={ref} onClick={() => setSeverityOpen(!severityOpen)} isExpanded={severityOpen}>
                      CVE severity {activeFilters.cveSeverity.length > 0 && <Label isCompact color="blue" style={{ marginLeft: 4 }}>{activeFilters.cveSeverity.length}</Label>}
                    </MenuToggle>
                  )}>
                  <SelectList>
                    {SEVERITIES.map((s) => (
                      <SelectOption key={s} hasCheckbox isSelected={activeFilters.cveSeverity.includes(s)} onClick={() => toggleFilter("cveSeverity", s)}>{s}</SelectOption>
                    ))}
                  </SelectList>
                </Select>
              </ToolbarItem>
              <ToolbarItem>
                <Select isOpen={cveStatusOpen} onOpenChange={setCveStatusOpen}
                  toggle={(ref) => (
                    <MenuToggle ref={ref} onClick={() => setCveStatusOpen(!cveStatusOpen)} isExpanded={cveStatusOpen}>
                      CVE status {activeFilters.cveStatus.length > 0 && <Label isCompact color="blue" style={{ marginLeft: 4 }}>{activeFilters.cveStatus.length}</Label>}
                    </MenuToggle>
                  )}>
                  <SelectList>
                    {CVE_STATUSES.map((s) => (
                      <SelectOption key={s} hasCheckbox isSelected={activeFilters.cveStatus.includes(s)} onClick={() => toggleFilter("cveStatus", s)}>{s}</SelectOption>
                    ))}
                  </SelectList>
                </Select>
              </ToolbarItem>
              ...
              <ToolbarItem>
                ...
                >
                  <DropdownList>
                    ...
                    <DropdownItem key="schedule" onClick={() => { setCreateReportOpen(false); navigate(routes.vulnerabilityReportsCreatePath, { state: { filters: activeFilters, savedFilterName: filtersModified ? null : (selectedSavedFilter || null), filtersModified } }); }} description="Schedule a recurring report based on current filters.">
                ...
                <ToolbarItem className="pf-v6-u-w-100">
                  <Flex gap={{ default: "gapSm" }} flexWrap={{ default: "wrap" }} alignItems={{ default: "alignItemsCenter" }}>
                    {Object.entries(activeFilters).map(([key, values]) =>
                      values.length > 0 ? (
                        <FlexItem key={key}>
                          <LabelGroup categoryName={formatCategoryName(key)} isClosable onClose={() => { setActiveFilters((p) => ({ ...p, [key]: [] })); setPage(1); }}>
                            {values.map((v) => <Label key={v} onClose={() => removeFilter(key, v)}>{v}</Label>)}
                          </LabelGroup>
                        </FlexItem>
                      ) : null
                    )}
        ...
        {/* Entity toggle (CVE / Image / Deployment) + pagination */}
        <Flex justifyContent={{ default: "justifyContentSpaceBetween" }} alignItems={{ default: "alignItemsCenter" }} style={{ marginTop: 16, marginBottom: 8 }}>
          ...
          <FlexItem>
            <Flex gap={{ default: "gapMd" }} alignItems={{ default: "alignItemsCenter" }}>
              <FlexItem>
                <Select isOpen={columnsOpen} onOpenChange={setColumnsOpen}
                  toggle={(ref) => (
                  ...
                  <SelectList>
                    {[
                      ...
                      <SelectOption key={key} hasCheckbox isSelected={visibleColumns[key]} onClick={() => setVisibleColumns((prev) => ({ ...prev, [key]: !prev[key] }))}>{label}</SelectOption>
                    ))}
                  </SelectList>
                </Select>
              </FlexItem>
        ...
        {loading ? (
          <div style={{ padding: 24 }}>
            {[...Array(5)].map((_, i) => <Skeleton key={i} width="100%" height="40px" style={{ marginBottom: 8 }} />)}
          </div>
        ) : activeView === "CVEs" ? (
          cveRows.length === 0 ? (
                    ...
                    <Tr key={cve.cve}>
                      <Td>
                        <Flex direction={{ default: "column" }} gap={{ default: "gapNone" }}>
                          <FlexItem><Button variant="link" isInline onClick={() => navigate(`${routes.cveDetailPath}/${cve.cve}`)}>{cve.cve}</Button></FlexItem>
                    ...
                    <Td>
                      <Flex direction={{ default: "column" }} gap={{ default: "gapNone" }}>
                        <FlexItem><Button variant="link" isInline onClick={() => {
                          ...
                          if (dep) navigate(`${routes.deploymentDetailPath}/${dep.id}`);
          ...
          <Table aria-label="Vulnerabilities table" variant="compact" gridBreakPoint="" isStickyHeader>
            ...
            <Tbody>
              {sortedDeployments.slice((page - 1) * perPage, page * perPage).map((row) => (
                <Tr key={row.id}>
                  {visibleColumns.deployment && <Td><Button variant="link" isInline onClick={() => navigate(`${routes.deploymentDetailPath}/${row.id}`)}>{row.name}</Button></Td>}
                  ...
                  {visibleColumns.cluster && <Td><Button variant="link" isInline onClick={() => navigate(routes.clustersPath)}>{row.clusterName}</Button></Td>}
      ...
      {/* Update saved filter confirmation modal */}
      <Modal variant="medium" isOpen={updateFilterModalOpen} onClose={() => setUpdateFilterModalOpen(false)}>
        <ModalHeader title={`Update saved filter — "${selectedSavedFilter}"?`} />
        <ModalBody>
          <Content><Content component="p">The current filter configuration will replace the saved filter settings.</Content></Content>
        </ModalBody>
        <ModalFooter>
          <Button variant="primary" onClick={async () => {
            const match = savedFilters.find((f) => f.name === selectedSavedFilter);
            if (match) {
      ...
      <Modal variant="medium" isOpen={createReportModalOpen} onClose={() => { setCreateReportModalOpen(false); setReportSuccess(false); }}>
        ...
        <ModalBody>
          <Flex direction={{ default: "column" }} gap={{ default: "gapMd" }}>
            <FlexItem><Content><p>Generate a CSV report based on this view and the filters you've applied.</p></Content></FlexItem>
            <FlexItem>
              <Card isCompact>
                <CardBody>
              ...
              <FlexItem>
                <Alert variant="success" isInline title="Report generation started successfully">
                  <Flex direction={{ default: "column" }} gap={{ default: "gapSm" }}>
                    ...
                    <FlexItem>
                      <Button variant="link" isInline icon={<ArrowRightIcon />} onClick={() => navigate(routes.vulnerabilityReportsPath)}>View status in reports table</Button>
                    </FlexItem>
                  </Flex>
                </Alert>
              </FlexItem>
        ...
        <ModalFooter>
          {!reportSuccess ? (
            <>
              ...
              <Button variant="link" onClick={() => setCreateReportModalOpen(false)}>Cancel</Button>
            </>
          ) : (
      ...
      <Modal variant="small" isOpen={saveFilterModalOpen} onClose={() => setSaveFilterModalOpen(false)}>
        ...
        <ModalBody>
          <Form>
            <FormGroup label="Filter name" isRequired fieldId="filter-name">
              <TextInput id="filter-name" value={newFilterName} onChange={(_, v) => setNewFilterName(v)} placeholder="Enter a name for this filter" />
            </FormGroup>
            <FormGroup label="Description" fieldId="filter-description">
              <TextInput id="filter-description" value={newFilterDescription} onChange={(_, v) => setNewFilterDescription(v)} placeholder="Optional description" />
            </FormGroup>
            <FormGroup label="Current filters" fieldId="current-filters">
              <div style={{ fontSize: 14, color: "var(--pf-t--global--text--color--subtle)" }}>
                {Object.entries(activeFilters).map(([key, values]) => values.length > 0 ? <div key={key}>{formatCategoryName(key)}: {values.join(", ")}</div> : null)}
              </div>
            </FormGroup>
          </Form>
        </ModalBody>
        <ModalFooter>
          <Button variant="primary" onClick={handleSaveFilter} isDisabled={!newFilterName.trim()}>Save filter</Button>
