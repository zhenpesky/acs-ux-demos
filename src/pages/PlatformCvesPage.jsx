...
export default function PlatformCvesPage({ routePrefix }) {
  ...
  const allCves = useMemo(() => PLATFORM_CVES.map((c) => ({
    cve: c.cve,
    severity: c.severity,
    cvss: c.cvss,
    type: c.type,
  ...
  const filteredCves = useMemo(() => {
    ...
    return stateFilteredCves.filter((c) =>
      [c.cve, c.severity, String(c.cvss), c.type, c.affectedComponents, c.fixedIn || '', c.firstDiscovered].some((field) =>
  ...
  return (
    <>
          ...
          <FlexItem>
            <Nav variant="horizontal-subnav" aria-label="Vulnerability management views">
              <NavList>
                ...
                <NavItem isActive itemId="platform" onClick={(e) => e.preventDefault()}>Platform</NavItem>
          ...
          <FlexItem>
            <Dropdown isOpen={moreViewsOpen} onSelect={() => setMoreViewsOpen(false)} onOpenChange={setMoreViewsOpen}
              ...
              <DropdownList>
                ...
                <DropdownItem key="k8s" isDisabled description="Vulnerabilities affecting the underlying Kubernetes infrastructure">Kubernetes components</DropdownItem>
              </DropdownList>
            </Dropdown>
          </FlexItem>
        </Flex>
      ...
      <PageSection>
        <Title headingLevel="h1">Platform Vulnerabilities</Title>
        <Content component="p">Vulnerabilities affecting platform components across clusters</Content>
      </PageSection>
      ...
      <PageSection>
        ...
        <Table aria-label="Platform CVEs" variant="compact">
          <Thead>
            <Tr>
              <Th>CVE</Th>
              <Th>Severity</Th>
