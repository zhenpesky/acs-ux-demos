  ...
  return (
    ...
    >
      <DropdownList>
        ...
        <DropdownItem key="clone" onClick={() => { setIsOpen(false); navigate(v2Routes.vulnerabilityReportsCreatePath); }}>Clone report</DropdownItem>
...
export default function V2ReportsPage() {
  ...
  const allReportConfigs = useMemo(() => {
    ...
    const created = createdReports.map((r) => ({
      id: r.id,
      name: r.name,
      collection: r.collection || r.savedFilter || 'Custom filters',
          ...
          <Tab eventKey={0} title={<TabTitleText>Report configurations</TabTitleText>}>
            <div style={{ padding: '16px 24px' }}>
              <Flex justifyContent={{ default: 'justifyContentSpaceBetween' }} alignItems={{ default: 'alignItemsCenter' }} style={{ marginBottom: 16 }}>
                ...
                <FlexItem>
                  <Button variant="primary" onClick={() => navigate(v2Routes.vulnerabilityReportsCreatePath)}>
                    Create report
                  </Button>
                </FlexItem>
              </Flex>
                      ...
                      <Tr key={row.id}>
                        ...
                        <Td dataLabel="Report">
                          <Button
                            variant="link"
                            isInline
                            onClick={() => navigate(`${v2Routes.vulnerabilityReportDetailPath}/${row.id}`)}
                          >
                            {row.name}
                          </Button>
                        </Td>
                        <Td dataLabel="Collection">
                          <Button variant="link" isInline onClick={() => navigate(v2Routes.collectionsPath)}>{row.collection}</Button>
                        </Td>
